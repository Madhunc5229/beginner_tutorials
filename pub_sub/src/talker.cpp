...
Talker::Talker(const std::string &node_name, std::string topic_name,
  ...
  RCLCPP_INFO_STREAM_ONCE(rclcpp::get_logger("rclcpp"), "Setting the message to 'hello' ");
  message_.text = "hello";
}
...
void Talker::addStrings(const std::shared_ptr<pub_sub::srv::AddTwoStrings::Request> request, std::shared_ptr<pub_sub::srv::AddTwoStrings::Response> response){
  ...
  RCLCPP_WARN_STREAM(rclcpp::get_logger("rclcpp"), "Incoming request\ntext1: "<< request->text1 <<" "<< "text2: "<< request->text2);  

  //using RCLCPP_DEBUG_STREAM to log the information about the respone from service
  RCLCPP_DEBUG_STREAM(rclcpp::get_logger("rclcpp"), "Sending back response: " << response->concatenated);
